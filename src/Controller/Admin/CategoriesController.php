...
class CategoriesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            if ($this->BlogCategories->save($category)) {
                $this->Flash->success('The category has been created successfully !');
    ...
    public function edit()
    {
        ...
        if (empty($category)) {
            $this->Flash->error('This category doesn\'t exist or has been deleted.');
            return $this->redirect(['action' => 'index']);
        }
        ...
        if ($this->request->is('put')) {
            ...
            if ($this->BlogCategories->save($category)) {
                $this->Flash->success('This category has been updated successfully !');
    ...
    public function delete()
    {
        ...
        if ($category->article_count >= 1) {
            $this->Flash->error('You can not deleted this category because one article or more is assigned to this category.');
            return $this->redirect(['action' => 'index']);
        }

        if ($this->BlogCategories->delete($category)) {
            $this->Flash->success('This category has been deleted successfully !');
            return $this->redirect(['action' => 'index']);
        }

        $this->Flash->error('Unable to delete this category.');
