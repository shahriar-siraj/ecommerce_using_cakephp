...
class PaypalComponent extends Component
{
    ...
    public function notify()
    {
        ...
        Log::error(__('Notify run...'), 'paypal');
        ...
        if (is_string($response) && strcmp($response, 'VERIFIED') == 0) {
            if ($paymentStatus == "Completed") {
                 ...
                 Log::error(__('we will save the order here...'), 'paypal');

                $custom['discount_id'] = isset($custom['discount_id']) ? $custom['discount_id'] : null;
                // We need to save the order as well because it has been paid
                $this->_saveOrder();
    ...
    public function _saveOrder()
    {
        ...
        $this->_controller->loadModel('Orders');
        $this->_controller->loadModel('OrderProducts');
        $user_id = $this->_session->read('Auth.User.id');

        $data = [
            'first_name' => $this->_session->read('Auth.User.first_name'),
            'last_name' => $this->_session->read('Auth.User.last_name'),
            'email' => $this->_session->read('Auth.User.email'),
            'phone' => $this->_session->read('Auth.User.phone'),
            'user_id' => $user_id,
            'billing_address' => $this->_session->read('Cart.user_billing.address'),
            'billing_suburb' => $this->_session->read('Cart.user_billing.suburb'),
            'billing_state' => $this->_session->read('Cart.user_billing.state'),
            'billing_post_code' => $this->_session->read('Cart.user_billing.post_code'),
            'delivery_address' => $this->_session->read('Cart.user_delivery.address'),
            'delivery_suburb' => $this->_session->read('Cart.user_delivery.suburb'),
            'delivery_state' => $this->_session->read('Cart.user_delivery.state'),
            'delivery_post_code' => $this->_session->read('Cart.user_delivery.post_code')
        ];

        // Then Create an order for each merchant
        foreach ($this->_session->read('Cart.stores') as $k => $v) {
            $data['shop_id'] = $v['store_id'];
            $data['shop_name'] = $v['store_name'];
            $data['subtotal'] = $v['subtotal'];
            $data['gst'] = $v['gst'];
            $data['total'] = $v['total'];
            $data['delivery_method_name'] = $v['delivery_method_name'];
            $data['delivery_method_price'] = $v['delivery_method_price'];
        }

        $data = $this->_controller->Orders->newEntity($data);
        $this->_controller->Orders->save($data);
    ...
    protected function _sendResponse()
    {
        ...
        Log::error(('Send response...'), 'paypal');
